...
/* package */ class NotificationCompatCompatV21 extends NotificationCompatCompatV14 {
    ...
    private boolean resolveRemoteViews(Context appContext, String packageName, Notification notification) {
        ...
        boolean hasOutsideApplicationInfo = packageInfo != null && packageInfo.applicationInfo != null;
        if (hasOutsideApplicationInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            getNotificationFixer().fixIcon(notification.getSmallIcon(), appContext, hasOutsideApplicationInfo);
            getNotificationFixer().fixIcon(notification.getLargeIcon(), appContext, hasOutsideApplicationInfo);
        } else {
