...
- Each Maps Group receives one MaskAccounts isolation environment. The runtime's numeric diagnostic
  ID is an engine detail, not a product identity, Android system user, or profile.

## Given / When / Then acceptance

Given the device has original Maps, Google Services Framework, Google Play services, and Google
Play Store installed, and a new `地圖 1` MaskAccounts GroupApp exists:
...
2. When `地圖 1` starts, then GSF, GMS and Play Store must be installed in the same Group environment
...
3. When Maps reaches its initial screen, then the visible Android activity must be a MaskAccounts
   ...
   be under the Group's engine-private data directory rather than the main-system
   Maps data directory.
...
7. When MaskAccounts is force-stopped and the Maps GroupApp is launched again, then its Group
   environment binding and guest data directory must remain unchanged.
...
- MaskAccounts copied the original Maps revision and its virtual PackageManager reported all three
  split names.
- Runtime diagnostics recorded engine environment ID `1` for `地圖 1`.
- GSF, GMS and Play Store were each reported ready in that Group environment.
...
- The Android PackageManager still has one original `com.google.android.apps.maps`; the GroupApp is
...
- Guest Maps, GMS, and GSF processes run under the MaskAccounts UID, while the main-system GMS UID
  remains separate.
- The Group environment owns the guest package data and its account database is stored inside
