...
### Group environment and lazy Google runtime

- **Given** a newly created empty Group with a dedicated isolation environment and Google service
  state `NOT_PREPARED`.
- **When** LINE was added.
- **Then** the Group advanced to `READY`, and installed GSF, GMS, and Play Store in that environment.
  The engine diagnostic ID observed in this historical run was 2. Deleting the temporary Group
  removed its metadata and
  `virtual/data/user/2` directory.

### Legacy shared-environment promotion

- **Given** migrated `LINE 1` still mapped to the legacy shared engine environment 0 with 162
  private files.
- **When** its Google runtime was prepared for the first time.
- **Then** it received a dedicated isolation environment, all 162 LINE private files were copied,
  the mapping recorded `legacyDataMigrated=true`, and the homepage showed `Google 服務就緒`.
- Only GroupApp data is copied. Legacy shared GMS data is intentionally not copied, preventing two
...
rename/state transitions, deletion isolation, and one-time promotion of legacy shared mappings.

## Remaining boundary

This acceptance proves Group persistence, navigation, lazy GMS preparation, environment cleanup,
